...
if result:
    ...
    # 划分训练集和测试集
    X_train, X_test, y_train, y_test = train_test_split(score_num.reshape(-1, 1), play, test_size=0.2, random_state=42)

    # 使用score_num预测play
    model_score_num = LinearRegression()
    model_score_num.fit(X_train, y_train)
    predicted_score_num = model_score_num.predict(X_test)
    r2_score_score_num = r2_score(y_test, predicted_score_num)
    print("R2 Score (score_num -> play):", r2_score_score_num)

    # 划分训练集和测试集
    X_train, X_test, y_train, y_test = train_test_split(follow.reshape(-1, 1), play, test_size=0.2, random_state=42)

    # 使用follow预测play
    model_follow = LinearRegression()
    model_follow.fit(X_train, y_train)
    predicted_follow = model_follow.predict(X_test)
    r2_score_follow = r2_score(y_test, predicted_follow)
    print("R2 Score (follow -> play):", r2_score_follow)

    # 划分训练集和测试集
    X_train, X_test, y_train, y_test = train_test_split(tags_encoded, play, test_size=0.2, random_state=42)

    # 使用tags预测play
    model_tags = LinearRegression()
    model_tags.fit(X_train, y_train)
    predicted_tags = model_tags.predict(X_test)
