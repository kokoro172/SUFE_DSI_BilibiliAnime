...
# 定义SQL语句
sql = "SELECT play, score_num, follow, tags FROM anime WHERE score != 0"
...
if result:
    ...
    X_train, X_test, y_train, y_test = train_test_split(
        np.column_stack((score_num, follow, tags_encoded)), play, test_size=0.2, random_state=42
    )

    # 使用岭回归模型预测play
    model_score_num = Ridge()
    model_follow = Ridge()
    model_tags = Ridge()
